...
function loadData() {
    ...
    Gamepg = createGraphics(width, height);
...
function draw() {
    ...
    if (gameStart) {
        // console.log("running");
        console.log(GameImageIndex);

        image(GameImageList[GameImageIndex], 0, 0, width, height);
        // GameImageList[GameImageIndex]=null;
        ++GameImageIndex;
        if(GameImageIndex==GameImageList.length){
            gameStart=false;
            endgame();
            var newArray=[];
            // GameImageList=newArray;
            for (let indexs = 0; indexs < GameImageList.length; indexs++) {
                var element = GameImageList[indexs];
                element=null;
            }
...
var GameImageList=[],gameStart=false,GameImageIndex=0;
function updateCanvas() {
    // clear();
    // var Gamepg = createGraphics(width, height);
    Gamepg.clear();
    if (haveFoggy) {
        // console.log("sucess");
        ...
        Gamepg.image(img, 0, 0, width, height);
        Gamepg.image(backgroundGraph, dx, dy, dWidth, dHight, dx, dy, dWidth, dHight);
        Gamepg.image(objectGraph, dx, dy, dWidth, dHight, dx, dy, dWidth, dHight);
        Gamepg.image(peopleGraph, dx, dy, dWidth, dHight, dx, dy, dWidth, dHight);
        Gamepg.image(peopleFoggyImg, dx, dy, dWidth, dHight);
        for (let HPi = 0; HPi < HPObject.length; HPi++) {
            ...
            if (obj["type"] == "HPandArmor") {
                ...
                if (hp > 10) {//30 40
                    ...
                    Gamepg.image(d10, dx + 0.3 * edgeToWidth, dy, edgeToWidth * 0.1, edgeToHeight * 0.15);
                    Gamepg.image(d, dx + 0.4 * edgeToWidth, dy, edgeToWidth * 0.1, edgeToHeight * 0.15);
                }
                else {  //35
                    // var d = imgObject[parseInt(imgDic[Math.floor(hp % 10).toString()])];
                    ...
                    if (hp < 0) {
                        ...
                        Gamepg.image(d, dx + 0.45 * edgeToWidth, dy, edgeToWidth * 0.1, edgeToHeight * 0.15);
                    }
                    else {
                        var d = imgObject[parseInt(imgDic[Math.floor(hp % 10).toString()])];
                        Gamepg.image(d, dx + 0.35 * edgeToWidth, dy, edgeToWidth * 0.1, edgeToHeight * 0.15);
                    }
                }
                if (armor > 10) { //60 70
                    ...
                    Gamepg.image(d10, dx + 0.65 * edgeToWidth, dy, edgeToWidth * 0.1, edgeToHeight * 0.15);
                    Gamepg.image(d, dx + 0.75 * edgeToWidth, dy, edgeToWidth * 0.1, edgeToHeight * 0.15);
                }
                else {
                    if (armor < 0) {
                        var d = imgObject[parseInt(imgDic["0".toString()])];
                        Gamepg.image(d, dx + 0.65 * edgeToWidth, dy, edgeToWidth * 0.1, edgeToHeight * 0.15);
                    }
                    else {
                        var d = imgObject[parseInt(imgDic[Math.floor(armor % 10).toString()])];
                        Gamepg.image(d, dx + 0.65 * edgeToWidth, dy, edgeToWidth * 0.1, edgeToHeight * 0.15);
                ...
                Gamepg.image(img, dx, dy, edgeToWidth, edgeToHeight * 0.15);
            }
            else if (obj["type"] == "HP") {
                /*777777 */
                ...
                if (hp > 10) {//40 50
                    var d10 = imgObject[parseInt(imgDic[Math.floor(hp / 10).toString()])];
                    var d = imgObject[parseInt(imgDic[Math.floor(hp % 10).toString()])];
                    Gamepg.image(d10, dx + 0.45 * edgeToWidth, ndy, edgeToWidth * 0.1, edgeToHeight * 0.15);
                    Gamepg.image(d, dx + 0.55 * edgeToWidth, ndy, edgeToWidth * 0.1, edgeToHeight * 0.15);
                }
                else {  //45
                    if (hp < 0) {
                        var d = imgObject[parseInt(imgDic["0".toString()])];
                        ...
                        var d = imgObject[parseInt(imgDic[Math.floor(hp % 10).toString()])];
                        Gamepg.image(d, dx + 0.45 * edgeToWidth, ndy, edgeToWidth * 0.1, edgeToHeight * 0.15);
                    }
                }

                Gamepg.image(img, dx, ndy, edgeToWidth, edgeToHeight * 0.15);
            }
        }
    }
    else {
        Gamepg.image(backgroundGraph, 0, 0, width, height);
        Gamepg.image(objectGraph, 0, 0, width, height);
        Gamepg.image(peopleGraph, 0, 0, width, height);
        for (let HPi = 0; HPi < HPObject.length; HPi++) {
            var obj = HPObject[HPi];
            var img = imgObject[parseInt(imgDic[obj["type"]])];
            var dx = obj["postion"][0] * edgeToWidth;
            ...
            if (obj["type"] == "HPandArmor") {
                ...
                if (hp > 10) {//30 40
                    ...
                    // var d = imgObject[parseInt(imgDic[Math.floor(hp % 10).toString()])];
                    // Gamepg.image(d, dx + 0.30 * edgeToWidth, dy, edgeToWidth * 0.1, edgeToHeight * 0.15);
                    if (hp < 0) {
                        var d = imgObject[parseInt(imgDic["0".toString()])];
                        Gamepg.image(d, dx + 0.45 * edgeToWidth, dy, edgeToWidth * 0.1, edgeToHeight * 0.15);
                    }
                ...
                if (armor > 10) { //60 70
                    ...
                    if (armor < 0) {
                        var d = imgObject[parseInt(imgDic["0".toString()])];
                        Gamepg.mage(d, dx + 0.65 * edgeToWidth, dy, edgeToWidth * 0.1, edgeToHeight * 0.15);
                    }
                    else {
                        var d = imgObject[parseInt(imgDic[Math.floor(armor % 10).toString()])];
                        Gamepg.image(d, dx + 0.65 * edgeToWidth, dy, edgeToWidth * 0.1, edgeToHeight * 0.15);
    ...
    GameImageList.push(Gamepg);
    // GameImageList.push("1");
    return true;
}
function loadCanvas() {
    // clear();
...
function ActionToImage(action_code){
    while(1){
        // console.log("123");
        if (!onChanging) {
            /*優化部分*/
