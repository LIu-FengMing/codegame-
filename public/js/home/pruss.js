...
function btnClick(number) {
  ...
  for (let index = 0; index < user.EasyEmpire.codeLevel.length; index++) {
    ...
    if (parseInt(element.level) == number) {
      codeLevel = parseInt(element.HighestStarNum);
    }
  }
  var blockLevel = -1;
  for (let index = 0; index < user.EasyEmpire.blockLevel.length; index++) {
    ...
    if (parseInt(element.level) == number) {
      blockLevel = parseInt(element.HighestStarNum);
    }
  }
  codeStar = codeLevel;
  blockStar = blockLevel;
...
function viewRecord(number) {
  ...
  divID = "centerBlocklyView";
  divID2 = "centerBlocklyBkView";
  b = document.createElement("div");
